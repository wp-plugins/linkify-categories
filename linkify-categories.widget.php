...
class c2c_LinkifyCategoriesWidget extends WP_Widget {
	private $widget_id = 'linkify_categories';
	private $textdomain = 'linkify-categories';
	private $title = '';
	private $description = '';
	private $config = array();
	private $defaults = array();

	public function c2c_LinkifyCategoriesWidget() {
		$this->title = __( 'Linkify Categories', $this->textdomain );
		...
		$this->config = array(
			...
			'title' => array( 'input' => 'text', 'default' => __( 'Categories', $this->textdomain ),
					'label' => __( 'Title', $this->textdomain ) ),
			'categories' => array( 'input' => 'text', 'default' => '',
					'label' => __( 'Categories', $this->textdomain ),
					'help' => __( 'A single category ID/slug, or multiple category IDs/slugs defined via a comma-separated and/or space-separated string.', $this->textdomain ) ),
			'before' => array( 'input' => 'text', 'default' => '',
					'label' => __( 'Before text', $this->textdomain ),
					'help' => __( 'Text to display before all categories.', $this->textdomain ) ),
			'after' => array( 'input' => 'text', 'default' => '',
					'label' => __( 'After text', $this->textdomain ),
					'help' => __( 'Text to display after all categories.', $this->textdomain ) ),
			'between' =>  array( 'input' => 'text', 'default' => ', ',
					'label' => __( 'Between categories', $this->textdomain ),
					'help' => __( 'Text to appear between categories.', $this->textdomain ) ),
			'before_last' =>  array( 'input' => 'text', 'default' => '',
					'label' => __( 'Before last category', $this->textdomain ),
			...
			'none' =>  array( 'input' => 'text', 'default' => '',
					...
					'help' => __( 'Text to appear when no categories have been found.  If blank, then the entire function doesn\'t display anything.', $this->textdomain ) )
		);

		foreach ( $this->config as $key => $value )
			$this->defaults[$key] = $value['default'];
		$widget_ops = array( 'classname' => 'widget_' . $this->widget_id, 'description' => __( 'Converts a list of categories (by slug or ID) into links to those categories.', $this->textdomain ) );
	...
	public function widget( $args, $instance ) {
		...
		foreach ( array_keys( $this->config ) as $key )
			$$key = apply_filters( 'linkify_categories_widget_'.$key, $instance[$key] );
